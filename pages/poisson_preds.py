...
def get_stats(year: int) -> dict:
    YEAR = year
    stats = list(db.fotmob_stats.aggregate([{"$match": {"general.season": {"$in": [f"{YEAR}", f"{YEAR-1}/{YEAR}"]}}}, 
...
def plot_venue_performances(df: pd.DataFrame, home_team: str, away_team: str) -> None:
    ...
    sns.barplot(data=df_home, x='home', y='weighted_performance_home', ax=ax, color='#1B2C6B')
    sns.barplot(data=df_away, x='away', y='weighted_performance_away', ax=ax, color='#6B381B')
    sns.despine()
...
league = st.selectbox(label='Select a League:', options=df['league'].unique(), index=0)
