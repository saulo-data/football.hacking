...
if st.session_state['logged_in']:
    ...
    def get_goals_metrics(
        ...
        column_team_home: str,
        home_team: str,
        column_home_scores: str,
        columns_team_away: str,
        away_team: str,
        columns_away_scores: str,
        total_home_avg: float,
        total_away_avg: float
    ) -> tuple[float]:
    ...
    # correction low scores (Dixon-Coles)
    ...
    def get_match_probs(goal_matrix: np.outer) -> dict:
        ...
        match_probs = {
            "Match Odds": {
                'Home': {'Prob': home_win_prob, 'Odd': np.round(100 / home_win_prob, 2)},
                "Draw": {'Prob': draw_prob, 'Odd': np.round(100 / draw_prob, 2)},
                "Away": {'Prob': away_win_prob, 'Odd': np.round(100 / away_win_prob, 2)},
            },
            "Double Chance": {
                "Home": {"Prob": home_double_chance_prob, "Odd": np.round(100 / home_double_chance_prob, 2)},
                "No Draw": {"Prob": any_win_double_chance_prob, "Odd": np.round(100 / any_win_double_chance_prob, 2)},
                "Away": {"Prob": away_double_chance_prob, "Odd": np.round(100 / away_double_chance_prob, 2)}
            },
            "Draw No Bet": {
                "Home": {"Prob": home_handicap_0, "Odd": np.round(100 / home_handicap_0, 2)},
                "Away": {"Prob": away_handicap_0, "Odd": np.round(100 / away_handicap_0, 2)}
            }
        }
        return match_probs
    ...
    def get_btts_probs(goal_matrix: np.outer) -> dict:
        ...
        btts = {
            "BTTS": {
                "Yes": {"Prob": btts_yes * 100, "Odd": np.round(1 / btts_yes, 2)},
                "No": {"Prob": btts_no * 100, "Odd": np.round(1 / btts_no, 2)}
            }
        }
        return btts
    ...
    def get_unders_overs(home_goals: float, away_goals: float) -> dict:
        ...
        for i in range(0, 6):
            ...
            over = 1 - under
            goals_probs[f"{i}.5"] = {
                "Over": {"Prob": over * 100, "Odd": np.round(1 / over, 2)},
                "Under": {"Prob": under * 100, "Odd": np.round(1 / under, 2)}
            }
    ...
    st.dataframe(
        df_league
        .drop(columns=['home_image', 'away_image'])
        .rename(columns={
            'league': 'League',
            'season': 'Season',
            'home': 'Home Team',
    ...
    if ideal_len(df_league) > len(df_league):
        st.warning(
            f"{league} still does not have enough matches for reliable predictability using the Poisson distribution. "
            "It is recommended to wait until there are more rounds played.",
            icon="🚨"
        )
    ...
    d_home, d_away, d_skellmam, d_total_goals, skew_league = get_dispersion_metrics(
        data=df_league,
        score_home='score_home',
        score_away='score_away',
        goals_sum='goals_sum'
    ...
    st.markdown("""
    Want to understand what these numbers mean in practice? Read the full breakdown here: 👉
    [How to Read Dispersion and Skew in Football Models](https://open.substack.com/pub/saulofaria/p/how-to-read-dispersion-and-skew-in?r=30n7hp&utm_campaign=post&utm_medium=web&showWelcomeOnShare=true)
    """)
    ...
    if submitted:
        ...
        home_goals, away_goals = get_goals_metrics(
            df_league=df_league,
            column_team_home='home',
            home_team=home,
            column_home_scores='score_home',
            columns_team_away='away',
            away_team=away,
            columns_away_scores='score_away',
            total_home_avg=total_home_avg,
            total_away_avg=total_away_avg
        )
        ...
        mc = mc_safety_match_odds(
            [match_odds_df.iloc[0, 0] / 100, match_odds_df.iloc[0, 1] / 100, match_odds_df.iloc[0, 2] / 100],
            kappa=kappa
        )['status']
