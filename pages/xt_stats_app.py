...
if st.session_state['logged_in']:
    ...
    if st.session_state['user']['plan'] == 'free':
        ...
        leagues = col_calendar.distinct('league')
    ...
    def get_names(venue: str, leagues: list) -> list[str]:
        teams = col_calendar.find({'league': {'$in': leagues}, 'season': {'$in': [2026, 2526]}}).distinct(f'{venue}_team')
        
        return teams
    
    # function to get match id
    @st.cache_data(ttl='12h', show_spinner=False)
    def get_match_data(home_team: str, away_team: str, leagues: list) -> int:
        match_data = list(col_calendar.find({'home_team': home_team, 'away_team': away_team, 'league': {'$in': leagues}, 'season': {'$in': [2026, 2526]}}, {'_id': 0, 'game_id': 1, 'league': 1, "home_team": 1, "away_team": 1, 
    ...
    def get_passes_df(match_id: int, xt_array: np.array) -> pd.DataFrame:
        ...
        'PSG': 'Paris Saint-Germain', 'Schalke': 'Schalke 04'}
