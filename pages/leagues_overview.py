...
if st.session_state['logged_in']:
    ...
    def get_data(league: str, season: str, country: str, teams: list) -> pd.DataFrame:
        ...
        df = pd.DataFrame({
            ...
            'Open-Play xG per 100 Passes': xg_for, 
            'Open-Play xG per 100 Passes Diff': xg_diff, 
            'Image': url_images
        })
    ...
    def get_team(season: str, country: str, league: str, teams: list) -> None:
        ...
        df2 = pd.DataFrame({
            "Round": round, 
            "Opponent": opponents, 
            'Open-Play xG per 100 Passes For': xg_for, 
            'Open-Play xG per 100 Passes Opp': xg_opp, 
            'Formation': formation, 
            'Opp Formation': formation_opp
        })
    
    
        df_grouped = df2.groupby(by=['Formation', 'Opp Formation'])[['Open-Play xG per 100 Passes For', 'Open-Play xG per 100 Passes Opp']].mean().reset_index()
    
        G = nx.Graph()
        G.add_nodes_from(df_grouped['Formation'].unique(), nodetype='b')
        G.add_nodes_from(df_grouped['Opp Formation'].unique(), nodetype='r')
        colors = [u[1] for u in G.nodes(data='nodetype')]
    
        for index, row in df_grouped.iterrows():
            diff = row['Open-Play xG per 100 Passes For'] - row['Open-Play xG per 100 Passes Opp']
            G.add_edge(row['Formation'], row['Opp Formation'], diff=np.round(diff, 2))
        ...
        # edges
        nx.draw_networkx_edges(G, pos, edgelist=elarge, width=6, alpha=.5, edge_color='b', ax=ax2)
        ...
        df3 = df2.copy()
        df3['Opp Formation'] = df3['Opp Formation'].str.replace('*', '')
    
        st.dataframe(df3, hide_index=True)
    
        st.subheader(f"{st.session_state['squad']} Formations (Blue) vs. Opponents’ Formations (Red)")
        st.write('Based on the average open-play xG difference per 100 passes')
    ...
    st.title(f'Open-Play xG per 100 Passes Charts')
    st.subheader('Select a league to analyze the open-play xG per 100 passes data')
    leagues_data = get_leagues_data(seasons=SEASONS, exclude=INT, leagues=leagues)
    ...
    df_plot = df[['Team', 'Passes Opp Half %', 'Open-Play xG per 100 Passes', 'Open-Play xG per 100 Passes Diff']]
    df_styled = df_plot.style.background_gradient(cmap='Greens', text_color_threshold=0.5, 
                                                                    subset=['Passes Opp Half %', 'Open-Play xG per 100 Passes'], low=0.1).format(precision=2).background_gradient(cmap='RdYlGn', 
                                                                                                                            text_color_threshold=0.5, 
                                                                                                                            subset=['Open-Play xG per 100 Passes Diff'], low=0).format(precision=2)
    
        
    #plot 1
    fig, ax = plt.subplots(figsize=(20, 8))
    ax.scatter(df['Open-Play xG per 100 Passes'], df['Open-Play xG per 100 Passes Diff'], c=background)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['bottom'].set_color('k')
    ax.spines['left'].set_color('k')
    ax.set_xlabel('Open-Play xG per 100 Passes', fontweight='bold', fontsize=12, color=text)
    ax.set_ylabel('Open-Play xG per 100 Passes Diff', fontweight='bold', fontsize=12, color=text)
    ax.tick_params(
        axis='both', 
        which='both', 
        colors=text,
    ...
    for index, row in df.iterrows():    
        ab = AnnotationBbox(get_image(row['Image']), (row['Open-Play xG per 100 Passes'], row['Open-Play xG per 100 Passes Diff']), frameon=False)
        ax.add_artist(ab)
    
        ax.hlines(df['Open-Play xG per 100 Passes Diff'].median(), df['Open-Play xG per 100 Passes'].min(), df['Open-Play xG per 100 Passes'].max(), 
                color=text, alpha=0.3)
        ax.hlines(np.percentile(df['Open-Play xG per 100 Passes Diff'], 25), df['Open-Play xG per 100 Passes'].min(), df['Open-Play xG per 100 Passes'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
        ax.hlines(np.percentile(df['Open-Play xG per 100 Passes Diff'], 75), df['Open-Play xG per 100 Passes'].min(), df['Open-Play xG per 100 Passes'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
    
    
        ax.vlines(df['Open-Play xG per 100 Passes'].median(), df['Open-Play xG per 100 Passes Diff'].min(), df['Open-Play xG per 100 Passes Diff'].max(), 
                color=text, alpha=0.3)
        ax.vlines(np.percentile(df['Open-Play xG per 100 Passes'], 25), df['Open-Play xG per 100 Passes Diff'].min(), df['Open-Play xG per 100 Passes Diff'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
        ax.vlines(np.percentile(df['Open-Play xG per 100 Passes'], 75), df['Open-Play xG per 100 Passes Diff'].min(), df['Open-Play xG per 100 Passes Diff'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
    
                
    #plot2
    fig2, ax2 = plt.subplots(figsize=(20, 8))
    ax2.scatter(df['Passes Opp Half %'], df['Open-Play xG per 100 Passes Diff'], c=background)
    ax2.spines['top'].set_visible(False)
    ax2.spines['right'].set_visible(False)
    ax2.spines['bottom'].set_color('k')
    ax2.spines['left'].set_color('k')
    ax2.set_xlabel('Passes Opp Half %', fontweight='bold', fontsize=12, color=text)
    ax2.set_ylabel('Open-Play xG per 100 Passes Diff', fontweight='bold', fontsize=12, color=text)
    ax2.tick_params(
        axis='both', 
        which='both', 
        colors=text,
    ...
    for index, row in df.iterrows():    
        ab = AnnotationBbox(get_image(row['Image']), (row['Passes Opp Half %'], row['Open-Play xG per 100 Passes Diff']), frameon=False)
        ax2.add_artist(ab)
    
        ax2.hlines(df['Open-Play xG per 100 Passes Diff'].median(), df['Passes Opp Half %'].min(), df['Passes Opp Half %'].max(), 
                color=text, alpha=0.3)
        ax2.hlines(np.percentile(df['Open-Play xG per 100 Passes Diff'], 25), df['Passes Opp Half %'].min(), df['Passes Opp Half %'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
        ax2.hlines(np.percentile(df['Open-Play xG per 100 Passes Diff'], 75), df['Passes Opp Half %'].min(), df['Passes Opp Half %'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
    
    
        ax2.vlines(df['Passes Opp Half %'].median(), df['Open-Play xG per 100 Passes Diff'].min(), df['Open-Play xG per 100 Passes Diff'].max(), 
                color=text, alpha=0.3)
        ax2.vlines(np.percentile(df['Passes Opp Half %'], 25), df['Open-Play xG per 100 Passes Diff'].min(), df['Open-Play xG per 100 Passes Diff'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
        ax2.vlines(np.percentile(df['Passes Opp Half %'], 75), df['Open-Play xG per 100 Passes Diff'].min(), df['Open-Play xG per 100 Passes Diff'].max(), 
                color=text, alpha=0.1, **{'ls': '--'})
    ...
    with tab1:
        st.subheader('Open-Play xG per 100 Passes For x Diff')
        st.pyplot(fig)
    with tab2:
        st.subheader('Passes Opp Half % x Open-Play xG per 100 Passes Diff')
        st.pyplot(fig2)
